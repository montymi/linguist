...
def main():
    ...
    parser.add_argument(
        "--output_file", type=str, default="output.wav", help="Output file for TTS"
    )
    parser.add_argument(
        "--archive", type=str, default=None, help="Archive directory for TTS"
    )
    parser.add_argument(
        "--whisper_model", type=str, default="base", help="Whisper model for STT"
    )
    ...
    listen_parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Flag to print the recognized text",
    )
    listen_parser.add_argument("--tag", type=str, help="Tag the recorded audio file")

    # Transcribe command
    transcribe_parser = subparsers.add_parser(
        "transcribe", help="Transcribe audio file to text"
    )
    transcribe_parser.add_argument(
        "--path", type=str, required=True, help="Path to the audio file to transcribe"
    )
    transcribe_parser.add_argument(
        "--print",
        action="store_true",
        default=True,
        help="Flag to print the transcribed text",
    )
    transcribe_parser.add_argument(
        "--tag", type=str, help="Tag the transcribed audio file"
    )

    # Archive command
    archive_parser = subparsers.add_parser(
        "list", help="List all recorded audio samples"
    )
